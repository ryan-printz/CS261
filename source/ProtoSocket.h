...
public:
	ProtoSocket & operator = (const ProtoSocket & lhs)
	{
		...
		m_disconnected			= lhs.m_disconnected;
		m_received				= lhs.m_received;
	...
	{
		m_connectionAttempts = new std::list<NetAddress>();
		m_disconnected		 = new std::list<NetAddress>();
		m_received			 = new std::list<Packet>();
	};
